...
def _process_caption_data(caption_file, image_dir, max_length):
    ...
    # data is a list of dictionary which contains 'captions', 'file_name' and
    # 'image_id' as key.
    data = []
    for annotation in caption_data['annotations']:
        image_id = annotation['image_id']
        annotation['file_name'] = os.path.join(
            image_dir, id_to_filename[image_id])
        data += [annotation]
...
def main():
    ...
    batch_size = 100
    # maximum length of caption(number of word). if caption is longer than
    # max_length, deleted.
    #TODO: Increase the length
    max_length = 15
    # if word occurs less than word_count_threshold in training dataset, the
    # word index is special unknown token.
    ...
    train0_cutoff = int(1.0 / 4 * len(train_dataset))
    train1_cutoff = int(2.0 / 4 * len(train_dataset))
    train2_cutoff = int(3.0 / 4 * len(train_dataset))
    train3_cutoff = int(4.0 / 4 * len(train_dataset))
    ...
    if not os.path.exists('data/train0'):
        os.makedirs('data/train0')
    if not os.path.exists('data/train1'):
        os.makedirs('data/train1')
    if not os.path.exists('data/train2'):
        os.makedirs('data/train2')
    if not os.path.exists('data/train3'):
        os.makedirs('data/train3')

    save_pickle(train_dataset,
                'data/train/train.annotations.pkl')
    save_pickle(train_dataset[:train0_cutoff],
                'data/train0/train0.annotations.pkl')
    save_pickle(train_dataset[train0_cutoff:train1_cutoff],
                'data/train1/train1.annotations.pkl')
    save_pickle(train_dataset[train1_cutoff:train2_cutoff],
                'data/train2/train2.annotations.pkl')
    save_pickle(train_dataset[train2_cutoff:train3_cutoff],
                'data/train3/train3.annotations.pkl')
    save_pickle(val_dataset[:val_cutoff], 'data/val/val.annotations.pkl')
    save_pickle(val_dataset[val_cutoff:test_cutoff].reset_index(
        drop=True), 'data/test/test.annotations.pkl')

    temp_anno = load_pickle('./data/train/train.annotations.pkl')
    word_to_idx = _build_vocab(
        annotations=temp_anno, threshold=word_count_threshold)
    save_pickle(word_to_idx, './data/train/word_to_idx.pkl')

    for split in ['train0', 'train1', 'train2', 'train3', 'val', 'test']:
        annotations = load_pickle(
            './data/%s/%s.annotations.pkl' % (split, split))
        ...
        for split in ['train0', 'train1', 'train2', 'train3', 'val', 'test']:
            ...
            for start, end in zip(range(0, n_examples, batch_size),
                ...
                image_batch = np.array(map(lambda x: ndimage.imread(x, mode='RGB'), image_batch_file)).astype(
                    np.float32)
                feats = sess.run(vggnet.features, feed_dict={
                                 vggnet.images: image_batch})
                all_feats[start:end, :] = feats
                print ("Processed %d %s features.." % (end, split))

            # use pickle to save huge feature vectors
